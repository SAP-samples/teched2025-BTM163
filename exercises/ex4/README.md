...
3. Place the test steps according to the flow which are reusable as per the business requirements.
4. Select the folder, click on the three dots at the end of the selected line and select the option "Convert to reusable test step"
5. In the following dialog, select the option "Manually set up business parameters later" and click button "Apply".
6. The folder and its content is now migrated to a Reusable Test Block. You can access it via click on the options button (with the three dots) and "Jump to reusable test step"
7. The reusable Test Block will open in a new tab. Select the folder and click again the options button (with the three dots). Click on "Create parameter (child to selected)".
